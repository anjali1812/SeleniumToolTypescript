...
function run_spec() {
   ...
   for (let i = 0; i < spec_array.length; i++) {
      ...
      let spec_run_data = spec_array[i] + " => " + getTimeStamp() + " => " + spec_array[i].split(" => ")[1].split(split)[name_index - 1].replace(".ts","");
   ...
   for (let i = 0; i < spec_array_with_result_folder.length; i++) {
      if (spec_array_with_result_folder[i].split(" => ").length == 4) {
         ...
         let final_result_folder = String("results/_parallel/" + spec_array_with_result_folder[i].split(" => ")[3] + "/" + spec_array_with_result_folder[i].split(" => ")[2]);
